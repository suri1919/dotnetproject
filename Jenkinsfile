pipeline {
  ...
  stages {
    ...
    stage('Docker build and push') {
      steps {
		sh '''
		 ...
		 docker tag docker-project1:latest 754495971822.dkr.ecr.ap-south-1.amazonaws.com/docker-project1:${BUILD_NUMBER}
    ...
    stage('Deploy docker'){
      steps {
		sh '''
			  ssh -i /var/lib/jenkins/.ssh/application.pem -o StrictHostKeyChecking=no ubuntu@ec2-65-1-136-62.ap-south-1.compute.amazonaws.com 'bash -s' < ./deploy.sh \${BUILD_NUMBER}
			  '''	
      		}
		}		    
